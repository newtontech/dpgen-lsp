...
def _lint_general_checks(data: dict, text: str) -> list[dict]:
    ...
    if (
        isinstance(numb_models, int)
        and numb_models > 0
        and isinstance(training_iter0_model_path, list)
    ):
    ...
    if isinstance(sys_configs, list) and isinstance(model_devi_jobs, list):
        ...
        for job_index, job in enumerate(model_devi_jobs):
            ...
            if isinstance(sys_idx, int):
                ...
                invalid.extend(
                    idx for idx in sys_idx if not isinstance(idx, int) or idx < 0 or idx > max_idx
                )
            if invalid:
                diagnostics.append(
                    _diagnostic(
...
def _lint_init_workflow(workflow: str, data: dict, text: str) -> list[dict]:
    ...
    if (
        workflow == "init_bulk"
        and isinstance(init_fp_style, str)
        and init_fp_style.upper() not in {"VASP", "ABACUS"}
    ):
        diagnostics.append(
            _lint_simple(
                "init_fp_style",
        ...
        if print_level is None or str(print_level).upper() not in ("MEDIUM", "LOW", "HIGH"):
            ...
            diagnostics.append(
                _diagnostic(
                    ...
                    category="suggestion",
                    fix_hints=["Add 'GLOBAL': {'PRINT_LEVEL': 'MEDIUM'} to fp_params"],
                    blocking=False,
                )
            )
